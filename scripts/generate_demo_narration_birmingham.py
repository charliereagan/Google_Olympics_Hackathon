#!/usr/bin/env python3
"""One-shot CLI: generate the Birmingham narration MP3 for the demo.

Mirrors `scripts/generate_demo_narration.py` exactly (same model ID,
same voice, same sentence-by-sentence chunking, same inline-tag
handling, same WAV->MP3 encode). Differences from the Mount Pleasant
script: parses the BIRMINGHAM fixture entry from `web/lib/story-fixture.ts`
and writes to a different output path.

Voice: Algenib (HOE-DEC-025).
Model: gemini-3.1-flash-tts-preview.

Output: /web/public/fixture/narration-birmingham-alabama.mp3

Cost: 5 paragraphs × ~25 sentences × short Pro TTS call ≈ ~$0.25.

Usage:
    .venv/bin/python scripts/generate_demo_narration_birmingham.py
"""
...
# The fixture entry we target — match the FIXTURE_BIRMINGHAM_ALABAMA
# const in story-fixture.ts.
FIXTURE_CONST_NAME = "FIXTURE_BIRMINGHAM_ALABAMA"
OUTPUT_PATH = (
    REPO_ROOT / "web" / "public" / "fixture" / "narration-birmingham-alabama.mp3"
)
FIXTURE_PATH = REPO_ROOT / "web" / "lib" / "story-fixture.ts"


def load_body_paragraphs() -> list[str]:
    """Pull `body_paragraphs` from a SPECIFIC fixture const in the TS file.

    With multiple BroadcastStory consts now in the fixture file, we can no
    longer anchor on a global `body_paragraphs:` marker. We slice out the
    block between `export const FIXTURE_BIRMINGHAM_ALABAMA: ... = {` and
    its closing `};`, then run the same body_paragraphs regex on that
    slice.
    """
...
async def amain() -> int:
    ...
    print(
        f"[generate_demo_narration_birmingham] "
        f"project={PROJECT} model={TTS_MODEL} voice={VOICE_NAME}"
    )
    print(f"[generate_demo_narration_birmingham] output={OUTPUT_PATH}")

    paragraphs = load_body_paragraphs()
    print(f"[generate_demo_narration_birmingham] paragraphs={len(paragraphs)}")
    ...
    print(
        f"[generate_demo_narration_birmingham] OK mp3={mp3_size:,} bytes "
