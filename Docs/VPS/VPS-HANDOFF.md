...
**Last updated:** 2026-05-02 by VPS Session 1 (Claude Opus 4.7 — Day 1; pre-build doc review with Charlie; eight tightening decisions ratified, demo seed prompt scope locked to live URL only, anchor pre-screen + Wire pre-seed + Narrator/music-bed pull-forwards added)

**Project countdown:** **9 days to internal deadline (Sunday May 10 EOD), 10 days to Devpost hard deadline (May 11 5:00pm PT / 8:00pm ET). Today is Day 1 (Saturday May 2).**
...
**Last verified:** 2026-05-02, VPS Session 1
...
- **Demo seed prompt locked AS LIVE-URL ARTIFACT ONLY:** *"Find me a Team USA hometown story I've never heard before."* The prompt is the live URL's hero CTA — judges who poke around get a working investigation. **It does NOT appear in the demo video.** A typed prompt in the video would collapse the system into a chatbot mental model and forfeit the autonomous-newsroom positioning. (See VPS-DEC-030.)
...
|---|---|---|---|
| `CONSTITUTION.md` | repo root | Drafted v1.1 (Pivot A+); Day-1 terminology clarifier added (first/next/newest OK) | VPS + Charlie |
| `PROJECT_BRIEF.md` | repo root | Drafted v1.1 (Pivot A+); Day-1 terminology clarifier + Updates/Discussions monitoring added | VPS + Charlie |
| `What_is_The_Storytellers_Room.md` | repo root | Drafted v1.1 (Pivot A+) | VPS + Charlie |
| `BUILD_SPEC.md` | `Docs/Engineering/BUILD_SPEC.md` | Drafted v1.2 (Pivot A+ + Day-1 tightening pass: seed-prompt-on-URL-only, Day 5 narrator test, Day 6–7 music sourcing, Day 1–2 Devpost outline, Wire pre-seed on URL load, Day 8-evening anchor pre-screen) | VPS + Charlie (HoE will iterate) |
| `HOE-HANDOFF.md` | `Docs/Engineering/HOE-HANDOFF.md` | Drafted HoE Session 0 baseline | HoE |
| `VPS-HANDOFF.md` | `Docs/VPS/VPS-HANDOFF.md` | This document | VPS |

### Engineering state

- No code shipped. No GCP project provisioned. No Cloud Run services. No BigQuery tables. No Firestore database. No demo URL. No agents. No tests.
- See `Docs/Engineering/HOE-HANDOFF.md` Section 2 for full engineering current-state.

### Demo state

- Storyboard locked (BUILD_SPEC §11). 3-minute video, 7 shot beats.
- **The demo video does NOT include the seed prompt or any user-typed input.** Wire → Floor → Equity-Editor-caused click into anchor → Broadcast → Evidence Drawer → return to Wire. The seed prompt lives only on the live URL hero. (VPS-DEC-030.)
- Anchor story will be selected on Day 9 from organic discoveries — NOT pre-planned (HOE-DEC-012 / VPS-DEC-008). **Day 8 evening soft-rank** of the top 3–5 candidates by emotional pull added as a backstop (VPS-DEC-027) so we don't arrive at Day 9 with no fallback if nothing lands.
- Voiceover framing: positive, never defensive.
- Music bed: 2-3 candidates sourced from Epidemic Sound or Artlist on **Day 6–7** (pulled forward from Day 8 — VPS-DEC-032), picked during Day 9 dress rehearsal under the actual Narrator audio of the anchor story.
- Narrator voice config tested on **Day 5** with a 30-second sample read (pulled forward from Day 6 — VPS-DEC-031). De-risk the emotional spine of the Broadcast before committing the rest of the production stack to it.
- Devpost text description outlined on **Day 1–2** (VPS-DEC-029); refined during Day 9 dress-rehearsal downtime; final pass Day 10.
...
- **Q1: How tightly should the Devpost text description focus on Challenge 2 alone vs. mentioning parity (Challenge 1) and LA28 momentum (Challenge 3)?** VPS recommendation: lead with Challenge 2 framing (hometown hubs, geographic correlation, place-as-protagonist) for clean Stage One vetting box, then add a paragraph noting that the architecture also addresses parity (the Equity Editor) and LA28 momentum (the trend signals in the Investigation Packet) as natural extensions. This gives judges multiple Impact-axis reasons to score high without diluting the Challenge 2 fit. Charlie to confirm before submission.
...
- **Q8: Scope-cut hierarchy under time pressure** — what gets cut first if Day 7–8 runs hot, and does the answer change between optimizing for Grand Prize ($15K) vs Challenge 2 win ($8K) vs Honorable Mention ($5K)? **Parked until Tuesday EOD (Day 4 of build).** By then we'll have real signal on what's behind, ahead, and at risk. VPS early lean (subject to Tuesday review): cut Floor polish before Broadcast polish; cut Tier 2 voice mode without hesitation; cut "Days to LA28" counter if the chrome takes more than 30 minutes; never cut Equity Editor visible interventions or the Evidence Drawer's NIL Redaction line.
