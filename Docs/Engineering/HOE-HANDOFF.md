...
**Last updated:** 2026-05-07, Day 6 close-out — **FOUNDATION PROVEN**. First-ever organic published_story doc landed in Firestore. Chain end-to-end real: Editor → Scout → Investigator → Storyteller → Equity Editor → Publish Gate (7 sub-stages, 29 claims checked) → Narrator → published_stories. Bounded organic-op mode locked (HOE-DEC-036) — runtime defaults OFF.
...
- **Live data in Firestore:** 17 lead_reports, 12 investigation_packets, 50 story_drafts, 1 publish_audit (`final_decision: cleared`), **2 published_stories** (first-ever organic — Tucson Desert Hub), 50 agent_handoffs (Worker D's structured event collection), and ~300+ wire_events. The pre-Day-7 over-redacted rows are still in `wire_events` but invisible to UI via the 1-hour window.
...
| HOE-DEC-034 | Cost ceilings layered: per-axis + per-day USD + absolute USD, all env-overridable | Day-6 chain debug exposed two binding issues: (a) per-axis Pro cap was 200K tokens/day — too tight; ~1 investigation tripped it. (b) USD ceiling didn't exist; only per-axis caps. Fix layered three guard rails in `agents/cost/counters.py`: per-axis (defense in depth) + daily USD ($50 default) + absolute USD ($300 default). Per-axis defaults bumped (gemini_pro 200K→5M, tts 200K→2M, etc.). All four ceilings overridable via env vars: `COST_CEILING_GEMINI_PRO_TOKENS`, `COST_CEILING_GROUNDING_QUERIES`, `COST_CEILING_DEEP_RESEARCH_CALLS`, `COST_CEILING_TTS_CHARS`, `COST_CEILING_DAILY_USD`, `COST_CEILING_ABSOLUTE_USD`. The room logs `*daily Pro cap reached, room is conserving*` on per-axis hit; logs `daily_usd=X >= limit=Y` on USD hit. | 2026-05-06 |
| HOE-DEC-035 | Cleared `publish_audits` carry an explicit `narration_dispatched: False` field at creation | Day-6 cleared-audit→Narrator wire (Worker E) requires the Editor to query `where('narration_dispatched', '==', False)` to find unprocessed audits. Firestore equality filters skip docs missing the field — so without explicit `False` at creation, the Editor never sees the audit. Fix in `agents/publish_gate/orchestrator.py::_write_publish_audit`: when `final_decision == 'cleared'`, payload sets `narration_dispatched: False` before write. The Editor's `dispatch_narrator` tool flips it to `True` after dispatch. Required composite index: `publish_audits:(final_decision, narration_dispatched, completed_at desc)` — provisioned 2026-05-06 (`CICAgJjF9oIK`, READY). Caught empirically Day-6 evening (existing audit was invisible to the Editor's scan). | 2026-05-06 |
| HOE-DEC-036 | The runtime defaults OFF outside bounded organic-op windows | Charlie's Day-6 evening directive: produce ~3-5 organic stories to back testing/dev, then stop. Don't burn ~$20/story producing 50 organic stories that will sit unused. Bounded run = 30-45 min runtime, ~$30-50 spend, 3-5 published_stories. Refresh corpus pre-demo (Day 9-ish) for 6-12 hours. Cloud Scheduler-driven autonomous loop deferred until pre-demo window. Reverses the original VPS "48-60 hour continuous op" plan. Saved as feedback memory. | 2026-05-06 |
...
15. **Pick the fan-out shape based on the task, not on a blanket rule.** Day-6 morning fanned out three workers in parallel; Charlie's laptop slept mid-run; all three stalled simultaneously at the 600s no-progress watchdog. Partial work landed (~30-50% per agent). Recovery: three sequential foreground workers, each completing one agent. The stall was a one-off host-machine pause — not an indictment of parallel fan-outs. **Default rule: fan out in parallel when tasks are independent and the time savings is meaningful (multi-hour deliveries). Use sequential when tasks have hard dependencies or when wall-time isn't the bottleneck.** When workers do stall, the recovery pattern (three sequential foreground workers, each completing one agent's missing pieces) is fast, clean, and lets the HoE review at each handoff.
...
### 2026-05-07 — Day 6 (foundation proven — first-ever organic published_story end-to-end)

> Note: numbering tag is "Day 6" per the project-internal day count (Days 1-7 = backend; Days 8-11 = frontend; Days 12-13 = consolidation; today is the strategic reframe per VPS-DEC-041 + the foundation-proving day).

| Change | Commit | Deployed |
|---|---|---|
| Day-5 audit (VPS) revealed: the entire visible product was fixtures. 0 candidates, 0 published_stories, 0 investigation_packets ever written despite 7 days of build. The architecture had not been proven end-to-end. The HoE flagged this in the morning report; VPS authorized fix-overnight rather than halt-broadly per Lesson 26. | (this session) | N/A |
| Six workers ran sequenced (A→B/C/D parallel→E surgical→inline HoE fixes). All ratified; tests green; chain reached `published_stories` at 22:59 UTC with the first-ever organic published story. | (this session) | N/A |
| **Worker A** (af23760eac994d165): three surgical fixes — (1) `agents/publish_gate/fact_check.py` 0-claims pass-through bug (explicit short-circuit at line 360); (2) `agents/cost/counters.py` USD ceilings (HOE-DEC-034 — `_DEFAULT_DAILY_USD=50`, `_DEFAULT_ABSOLUTE_USD=300`, env overrides); (3) `scripts/probe_full_chain.py` `--first-lead-budget` default 60s→150s. 8 new tests pass; 273 existing pass; 0 regressions. | (this session) | N/A |
| **Worker B** (a22d9694a1cec7f0f): VPS-DEC-038 rename `/floor` (places-constellation) → `/field`. Files renamed (`Floor.tsx → Field.tsx`, `floor-fixture.ts → field-fixture.ts`, `app/floor/ → app/field/`). All identifiers swapped (Floor→Field, FloorNode→FieldNode, FLOOR_NODES→FIELD_NODES, etc.). `INTERVENTION_NODE_ID = 'birmingham-al'` preserved. Lineage breadcrumb left in code: *"(formerly /floor; VPS-DEC-038 freed /floor for the BUILD_SPEC §9 agent graph)"*. `/field` returns 200; `/floor` returns 404. Smart preservation: `Math.floor` comments untouched. | (this session) | ✓ live |
| **Worker C** (a8d7be446a2376db1): VPS-DEC-044 Broadcast autoplay-with-mute. State machine `loading → autoplaying / direct-link-fallback → playing/paused`. Two-step play attempt: `audio.play()` → on rejection `audio.muted=true` then retry. 24×24 hand-drawn SVG mute toggle, 44×44 tap target. "BEGIN BROADCAST" 56×56 play overlay with hairline gold-warm circle for direct-link arrivals. sessionStorage key `storytellers.muted` persists across same-tab nav, clears on full reload. Reduced-motion respected. Single file modified: `web/components/AudioBar.tsx` (+238 LOC). | (this session) | ✓ live |
| **Worker D** (a2b841da84370d02f): SSE handoff-event backend (R4). New `agents/handoffs.py` (209 LOC) with `emit_handoff` + `safe_emit_handoff` + 7-cast enum. New Firestore collection `agent_handoffs` schema: `{id, from_agent, to_agent, tool_call_id, story_unit_id, investigation_id, timestamp, mode}`. 10 dispatch wires across `editor/agent.py`, `equity_editor/tools.py`, `storyteller/tools.py`, `publish_gate/orchestrator.py`. SSE bridge route handler now emits a SECOND `onSnapshot` listener for `agent_handoffs` with same 1-hour window + Last-Event-ID resume; new SSE event types `handoff` and `handoff-preseed`. Required Firestore composite index `agent_handoffs:(mode asc, timestamp asc)` provisioned (`CICAgJiUpoMK`, READY). 28 new tests + 273 existing all pass. | (this session) | ✓ live |
| **Worker E** (a3cd91fc30f12e18f): cleared-audit → Narrator dispatch → published_stories. Editor scans `publish_audits` for `final_decision == 'cleared' && narration_dispatched == False`, surfaces results in the prompt context as `cleared_audits_awaiting_narration`. One-line prompt instruction tells the Pro model to dispatch the Narrator on each. `dispatch_narrator(draft_id, voice_profile, audit_id)` extended with `audit_id`. Voice alias `algenib` → `broadcast` resolved at tool boundary. After dispatch, audit doc gets `narration_dispatched=True, narration_dispatched_at=<iso>`. Narrator's `narrate(...)` extended with `audit_id` kwarg → `_persist_published_story` writes a `published_stories` doc with full schema (kicker, headline, dek, body_paragraphs, pull_quote, verified_claims, narration metadata, hero_image_url, **nil_signature carried over from the audit**, audit_id backref, published_at, mode='published'). 9 new tests + 182 existing all pass. | (this session) | ✓ live |
| **HoE inline fixes** (during workers): (1) per-axis Pro cap bumped 200K→5M tokens/day with env overrides for all 4 axes (gemini_pro / grounding / deep_research / tts) + helper `_ceilings_with_env_overrides()`; (2) `agents/publish_gate/orchestrator.py::_write_publish_audit` writes `narration_dispatched: False` on cleared audits at creation (HOE-DEC-035 — fixes Worker E's silent-failure gap); (3) backfilled the 1 existing cleared audit with the field; (4) provisioned 2 Firestore composite indexes (`agent_handoffs:(mode, ts)` + `publish_audits:(final_decision, narration_dispatched, completed_at desc)`); (5) granted Vertex AI service agent `roles/storage.objectViewer` on all 3 GCS buckets (visual_review was hitting `FAILED_PRECONDITION`); (6) bumped runtime env `COST_CEILING_DAILY_USD=200` for tonight's debug session (overrides default $50). | (this session) | ✓ live |
| **End-to-end Day-6 verification — first-ever organic published_story:** at 22:59 UTC the chain landed `published_stories` doc — *"The Tucson Desert Hub Operates as a Dual Engine for Endurance Training"* — kicker `Tucson, Arizona`, narration Algenib 5:16 at `gs://storytellers-room-audio/jZz3aF6fof3ehDEmcxsk/0000.wav`, audit_id `56e6affa56a440c3...`, **nil_signature `claims_checked=29, softened=1, removed=0`** (Lesson 21's outcome of well-disciplined compliance). 4 body paragraphs. Real organic discovery via the Editor → Scout Desk → Investigator → Storyteller → Equity Editor cleared → Publish Gate cleared (7 sub-stages, 29 claims) → Narrator chain. Halt trigger fully closed. | (this session) | ✓ live |
| Day-6 spend: **~$60-70 of Vertex AI** (USD ceiling tripped at $51.64 mid-evening, bumped to $200 wall to finish debug; runtime stopped at end of session — 0 spend until Day 7). Cumulative project spend across all 7 build days well under $100. | (this session) | ✓ |
| Test count: 293 → **341 passing** (+48 tonight: 8 Worker A + 28 Worker D + 9 Worker E + 3 fact_check 0-claims). Lint clean across 51+ files. Apache 2.0 badge confirmed visible on GitHub repo About sidebar. | (this session) | ✓ |
| **Foundation proven. The architecture is real.** All 7 cast members fired in real organic operation. NIL Layer demonstrated visible work in an audit footer (Lesson 21 closed). Equity Editor cleared organically. Bounded organic-op mode locked (HOE-DEC-036) — runtime defaults OFF until Day 7's bounded run. | (this session) | ✓ live |
...
| Change | Commit | Deployed |
|---|---|---|
| Three Day-7 workers fanned out in parallel: Visualizer + Visual Review (replaces Day-6 stub), full NIL Redaction Layer (replaces Day-2 stub), end-to-end smoke harness. All three landed cleanly with overlapping `__init__.py` + `runtime.py` edits in distinct sections. **Day-6 lesson 15 vindicated — parallel works fine for short independent tasks; Day-6's stall was a one-off laptop-sleep, not a code-conflict pattern.** | (this commit) | N/A |
| **Visualizer + Visual Review** (worker a465eee290766c62c): `agents/publish_gate/visualizer.py` (917 LOC) — `Visualizer` class with `generate_hero` (Nano Banana Pro), `generate_hometown_panel` + `generate_historical_echo` (Flash Image), `generate_assets` (parallel via asyncio.gather), `fallback_hero` (HOE-DEC-020). Three §7.4 prompt templates VERBATIM with explicit Law 6 negative phrasing; 4 stricter-clause variants for regeneration escalation. `agents/publish_gate/visual_review.py` (441 LOC, replaces 30-LOC Day-6 stub) — Pro vision check returning `{is_photoreal, has_likeness, detected_marks, confidence}` per image, fail-closed on vision-call failure (defaults to regenerate). Orchestrator now calls `_generate_and_review_visuals` (regeneration loop with fallback at 4th failure). 17 new tests + 2 orchestrator tests. | (this commit) | N/A |
| **Full NIL Redaction Layer** (worker aa242b98d69efc390): `agents/publish_gate/nil_redaction_layer.py` (775 LOC, replaces stub usage) — disambiguation pass (min_needle_length≥4, word-boundary discipline, 50-char sport-context window for common given names), near-id Flash-Lite check (broadcast surface only, gated on direct-match==0 + text>100 chars + cost ceiling), small-aggregate check (regex + registry cross-ref, replaces 3-4 named athletes with count), `scan_broadcast(text)` async variant for the publish pipeline. Day-2 stub stays in place at `nil_redaction_layer_stub.py` as a fallback (kept importable as `NilRedactionLayerStub`). 26 new tests including ALL the Day-3/4 over-redaction regressions: "Monitoring" → pass (FIXED), "Chula Vista" → pass (FIXED), "Lake Placid" → pass (FIXED). `agents/wire/types.py` extended with `'return'` decision + 7 new audit-log fields (disambiguation rejections, near-id outcomes, aggregate counts). | (this commit) | N/A |
| **End-to-end smoke harness** (worker a62ea536bae56870f): `tests/integration/test_full_chain_e2e.py` (625 LOC, 3 tests) + `tests/integration/_chain_stubs.py` (525 LOC helper module — `FsClient` in-memory Firestore, ADK Runner stub with lazy-args resolution, synthetic 600-row registry). `scripts/probe_full_chain.py` (547 LOC, executable, ~$0.50-1.00/run cost ceiling documented at top). Tests cover: (1) full Lead → Investigator → Storyteller → Equity Editor → Publish Gate (7 sub-stages) → Narrator with NarrationManifest output; (2) Storyteller revision loop on equity return; (3) kill on max revisions reached. Real `WireEmitter` + real Day-2 NIL stub used (proves the proxy + Layer fire on every emit). Probe self-checks: `--help` renders; `--dry-run --url http://nonexistent.example` exits 3 cleanly. 7 probe-script tests. | (this commit) | N/A |
| HoE one-fix during review: Visualizer's default httpx timeout was 60s; Day-7 live Nano Banana Pro smoke timed out at 60s. Bumped to **120s default + 120s on the underlying httpx.AsyncClient**. Re-test still timed out at 120s — flagged in backlog as "Vertex AI image-gen latency variance"; the HOE-DEC-020 Day-9 pre-cache pattern (render heroes ahead of demo time) makes real-time latency a non-issue for the demo. The Visualizer's structure is correct (10 unit tests pass; mirrors working TTS client). | (this commit) | N/A |
| **End-to-end Day-7 smoke test (live infra):** boot completed cleanly with Visualizer + full NIL Layer + Visual Review wired in. `/health/agents` shows ALL 7 cast members idle. `/health/nil` shows registry_size=11188 (full Layer loaded with cost-counter attached for the Flash-Lite near-id calls). All 286 unit tests + 1 skipped (Firestore emulator integration) pass under both default + `-p no:randomly`. Direct Visualizer round-trip blocked by Nano Banana Pro latency (variable; will work via Day-9 pre-cache). | (this commit) | ✓ live |
| Test count: 238 → **293 passed** (+55 from Day-7: 17 visualizer + 7 visual_review + 26 NIL + 3 full-chain e2e + 7 probe + 2 orchestrator). Lint: 0 violations across 51 files. | (this commit) | ✓ |
| **Backend complete.** All 7 cast members alive; all 7 Publish Gate sub-stages real (no stubs); full NIL Layer with disambiguation; live image-gen + TTS verified. **Next: Day-8 frontend work, gated on the design-approach checkpoint per memory feedback** (Charlie wants to discuss design before any /web/ work begins). | (this commit) | N/A |

### 2026-05-04/05 — Day 6 (Equity Editor + Storyteller + Publish Gate alive — full 7-agent cast operational)

| Change | Commit | Deployed |
|---|---|---|
| Three Day-6 workers fanned out in parallel; **all three stalled simultaneously** at the 600s no-progress watchdog when Charlie's laptop slept (not a code issue — the host machine paused). Partial work landed: Equity Editor (`tools.py` 846 LOC + `__init__.py`), Storyteller (`__init__.py` + `types.py`), Publish Gate (`parity_review.py` + `source_review.py` + `types.py`). | (none) | N/A |
| HoE recovery: spawned three SEQUENTIAL foreground workers (one at a time, not parallel) to complete each agent. Smaller scope per worker; less stall risk; clean review at each handoff. **All three completed cleanly.** Lesson 15 added to handoff. | (this commit) | N/A |
| **Equity Editor** (recovery worker a578d7aab7cf4802b): `agents/equity_editor/agent.py` (743 LOC) + `prompts/equity_editor.md` (166 lines) + 13 tests. Pro-tier ADK Runner; 6 closure-bound tools (read_published_feed, read_draft, intervene_feed_drift, return_draft, clear_draft, block_draft) + pull_vocabulary. `review_feed()` and `review_draft(draft_id)` entry points; no autonomous loop (invoked by Editor). Failure-mode events emit `intervention` not `thinking` — interventions arrive (BUILD_SPEC §6.5). Editor's `accept_equity_recommendation` real impl (was stub) + new `request_equity_review(scope)` tool. One-line docstring tweak in `tools.py` to remove a literal `firestore.add('wire_events', ...)` token tripping CI lint from inside a docstring. | (this commit) | N/A |
| **Storyteller** (recovery worker aaa106537aa18cd93): `agents/storyteller/agent.py` (1,154 LOC) + `agents/storyteller/tools.py` (662 LOC) + `prompts/storyteller.md` (246 lines, the most rigorous prompt in the build) + 13 tests. Pro-tier ADK Runner; 4 closure-bound tools (read_investigation_packet, write_story_draft with structural-envelope validation per BUILD_SPEC §5.5, request_equity_review, request_publish_gate) + pull_vocabulary. `_DraftValidationError(field, message)` for the revision loop. `write_story()` runs the full revision loop: invoke Runner → equity review → publish gate → on `returned`, re-invoke Runner with feedback (max 3 revisions); on `blocked`, kill story; on cleared, mark draft.publish_gate_decision='cleared'. Editor's new `dispatch_storyteller(packet_id)` and `dispatch_narrator(draft_id, voice_profile)` tools. The Storyteller's prompt forbidden-words list survives ADK's instruction-template engine cleanly (HOE-DEC-032 sanitization works as designed). | (this commit) | N/A |
| **Publish Gate** (recovery worker ada05897f2baf6a1e): `agents/publish_gate/{fact_check,safety_review,language_review,visual_review,orchestrator}.py` (1,155 LOC orchestrator + 464 fact_check + 348 safety + 287 language + 30 visual stub) + `prompts/publish_gate.md` (110 LOC) + 25 tests across 6 test files. Sub-stage roster: 1=FactCheck (Pro structured-extraction with regex pre-pass for finish-times/scoring), 2=SourceReview (deterministic), 3=ParityReview (deterministic), 4=NIL Layer (Day-2 stub), 5=SafetyReview (Flash-Lite quote+private-info checks with regex fallback), 6=LanguageReview (pure-Python regex with context-aware fighter/despite + encouraged-temporal overlap filter), 7=VisualReview (Day-6 auto-pass stub; Day-7 ships the real Visualizer integration). Orchestrator runs all 7 in order, returns drafts on failure (max 3 revisions), kills with reason on revision-cap, writes PublishAudit on clear. Editor's new `dispatch_publish_gate(draft_id)` tool. Worker chose `google-genai` direct calls (not ADK Runner) for the Pro structured-extraction (Fact Check) and Flash-Lite (Safety Review) one-shot calls. | (this commit) | N/A |
| **End-to-end Day-6 smoke test (live infra):** boot completed cleanly; `/health/agents` shows ALL 7 CAST MEMBERS IDLE: editor, scout_desk, investigator, equity_editor, storyteller, narrator, publish_gate. `_build_placeholder_agents` names list is now empty — no more `'shell'` statuses. **The Storyteller's Room is fully cast.** | (this commit) | ✓ live |
| Test count: 181 → **238 passed** (+57 from Day-6: 13 equity + 13 storyteller + 25 publish_gate + 6 dispatch tests in editor). Lint: 0 violations across 49 files. | (this commit) | ✓ |

### 2026-05-04 — Day 5 (Narrator alive + Investigator visibility fix + vocab-tool gap closed)

| Change | Commit | Deployed |
|---|---|---|
| Three workers fanned out: (1) Narrator agent body — Gemini Flash TTS + NarrationManifest; (2) Investigator vocabulary tool-surface gap (turned out to be a no-op since the Day-4 worker had already wired it); (3) Investigation-packet write-failure visibility fix. All three landed without conflicts. | (this commit) | N/A |
| **Narrator body** (worker a78281e0e9d8ce781): `agents/narrator/{__init__,types,tts_client,audio,agent,test_audio,test_tts_client,test_agent}.py` (~1,640 LOC). NarrationManifest types pinned per BUILD_SPEC §7.6. `GeminiFlashTTSClient` uses async httpx against `gemini-3.1-flash-tts-preview` on global v1beta1; auth via google-auth ADC token. `NarratorAgent.narrate(draft, voice_profile)` synthesizes per-sentence chunks, wraps PCM in WAV, uploads to GCS `storytellers-room-audio` bucket, computes word timings via per-sentence linear interpolation (BUILD_SPEC §3.5 fallback path 2 — Gemini TTS API doesn't return word timings natively), detects place-name + era-reference cue offsets, returns NarrationManifest. Inline tags (`[short pause]`, `[long pause]`, `[emphasis]`) injected at sentence/paragraph boundaries + place-name first occurrence. Cost ceiling pre-check axis='tts'. Failure modes per §17.5: TTS error → retry once with simplified text → fall back to pre-rendered MP3 (Day-9 pre-cache). 28 unit tests. `prompts/narrator.md` documents voice + pacing rules + constraints. | (this commit) | N/A |
| **Investigator vocab tool-surface gap** (worker a070b32facfc1bcc7): turned out to be a non-issue — the Day-4 Investigator worker had already wired `pull_vocabulary` into the Investigator's `_bind_tools` and updated `prompts/investigator.md`. Worker added one new test: `test_pull_vocabulary_tool_uses_investigator_key` (asserts the tool keys into the `"investigator"` JSON bucket, not `"investigator_scout"`). | (this commit) | N/A |
| **Investigation-packet write-failure visibility fix** (worker aec37e7d08d70f9a4): diagnosis after Day-4's "0 packets at 90s" observation = scenario A (cycle just takes 2-5 minutes; the backlog entry already captured this) **plus** a latent silent-failure bug in `tools.py::_make_write_investigation_packet` — if Firestore `.add` raised, the exception was swallowed and the function returned a fake locally-generated UUID, so the model believed it succeeded but no packet ever landed. Violated BUILD_SPEC §17.1 ("fail visibly on the Wire"). Fix: tool now emits a Wire `thinking` event ("hold — investigation packet write failed; the room will retry on the next dispatch") on Firestore-write failure, plus INFO-level boundary logs around the persist call. Model-facing return contract unchanged. New test added. | (this commit) | N/A |
| **End-to-end Day-5 smoke test (live infra):** boot completed; `/health/agents` shows **`narrator: idle`** (was 'shell' Day-4). 5 of 7 cast members operational (editor, scout_desk, investigator, narrator + scout_desk wraps 4 sub-scouts; equity_editor/storyteller/publish_gate still shells). **Live Algenib TTS round-trip:** synthesized 14-word test sentence to 293,760 bytes of `audio/l16; rate=24000; channels=1` PCM (~6.12s of audio at 24kHz/16-bit/mono) in ~2s. The Narrator's TTS client works against live Vertex AI. | (this commit) | ✓ live |
...
**Last set:** 2026-05-07 (Day 6 close-out — foundation proven; bounded organic-op mode locked; frontend reframe authorized for Day 7+)

**Status boundary:** Backend works end-to-end on real organic data (first published_story landed today). Frontend has 4 surfaces shipped at fixture quality but the **Day-5 reframe per VPS-DEC-041 / 042 / 043 / 044 is the new direction** — fan engagement product first, demo storyboard downstream. Two production-quality fixture stories (Mount Pleasant, Park City, Birmingham) + 1 organic published_story (Tucson Desert Hub) currently in Firestore. Runtime stopped pending bounded-run authorization tomorrow.

**What's next (Day 7 plan — bounded organic op + frontend reframe):**

1. **Dedup fix** (15 min worker) — Editor wrote 2 published_stories docs for the same draft because `narration_dispatched=True` write raced against the next think cycle. Fix via Firestore transaction or stronger atomic guard on the dispatch decision.
2. **Bounded organic run** (30-45 min, ~$30-50) — restart runtime with `COST_CEILING_DAILY_USD=75` (a wall, not a target), fire 4-6 distinct seed prompts across diverse story angles, let chain produce ~3-5 organic published_stories, stop. End-state: 3-5 organic stories backing the frontend reframe alongside the 3 fixture stories.
3. **Frontend reframe** (Day 7-8, real organic data backs everything):
   - VPS-DEC-041 — new `/`: editorial masthead + ambient Wire ticker band + full-bleed cinematic story-hero + 3-card discovery row (THE MAP / THE FIELD / THE STORIES) + seed-prompt CTA + recent-stories grid + bottom-fixed nav. Move existing root content to `/wire`.
   - VPS-DEC-042 — new `/map`: D3-geo + TopoJSON US states (NO third-party tiles), painterly editorial palette, gold-warm place dots sized by Olympian/Paralympian count, hover→tooltip, click→Broadcast.
   - VPS-DEC-043 — `/story` facets (sport / era / type, URL query params).
   - BUILD_SPEC §9 — new `/floor` agent-graph: D3 force on Canvas + particle handoffs consuming Worker D's `agent_handoffs` SSE stream + tool call cards.
   - VPS-DEC-035 — make sure new `/` design is what's on screen during the demo's 0:05–0:10 establishing shot (HoE delivers; VPS owns recording).
4. **Cloud Run deploy** (Day 8-9) — `agent-runtime` + `web` services. Migrate hero PNGs + narration MP3s from `web/public/fixture/` to GCS. Both services with `--min-instances=1 --cpu-always-allocated --use-http2 --timeout=3600s` per BUILD_SPEC §3.7. Production URLs for submission.
5. **Pre-demo refresh** (Day 9, 6-12 hours) — restart runtime with polished system to refresh the organic corpus. Whatever lands becomes "what the room found" for the demo recording.
6. **Demo recording + submission** (Day 9-10) — per VPS-DEC-035 / 036 / 037. Submission deadline May 11 5pm PT; internal target May 10 EOD.

**What's frozen for the rest of the build:**

- The 7-agent cast architecture
- NIL Layer + 1-hour SSE window + emit proxy
- Counter math (UTC-midnight day-diff)
- Equity Editor no-apology rule + tests
- All 11 design tokens + 9 type-scale entries + room cubic-bezier
- The 3 fixture stories (Mount Pleasant + Park City + Birmingham) — locked content
- Cost ceiling architecture (per-axis + USD + env overrides per HOE-DEC-034)
- Cleared-audit → narrator → published_stories wire (HOE-DEC-035)
- Bounded organic-op mode (HOE-DEC-036)
