...
| Two known gaps logged in backlog: (1) `firestore_v1.AsyncCollectionReference.on_snapshot` raises NotImplementedError — HND detector falls back to stub mode at boot; Day-3 fix is the sync watcher on a thread per plan §G.2. (2) ADK UserWarning + authlib deprecation warning, both cosmetic. | (this commit) | N/A |
