...
15. **Parallel-worker fan-outs are vulnerable to host-machine pause.** Day-6 morning fanned out three workers in parallel; Charlie's laptop slept mid-run; all three stalled simultaneously at the 600s no-progress watchdog. Partial work landed (~30-50% per agent). Recovery: spawn THREE SEQUENTIAL foreground workers (one at a time) to complete each agent's missing pieces. Smaller scope per worker = less risk if anything else goes wrong; clean review at each handoff. Total wall-time was longer than parallel would have been (had it succeeded), but the work shipped and was reviewed cleanly. **For multi-day-of-work fan-outs, sequential is more resilient than parallel** — the parallel speed-up is wasted if any worker stalls. Reserve parallel fan-outs for short tasks (<10min wall-time per worker).
