...
| Cloud Run | **Still empty.** Both services local-only on `localhost:3000`. **Day-12 deploy is the last major infra task before submission.** | — |
...
Through Day 11: total Vertex AI spend remains modest. Day-11 image-gen + TTS for the 3 demo stories ≈ **$0.85** (3× Nano Banana Pro at ~$0.05 + ~10 minutes of Algenib TTS at ~$0.20-0.30 per minute). Cumulative Vertex AI spend across all 11 days well under the $100 informational threshold. Budget alerts active per BUILD_SPEC §15.2:
